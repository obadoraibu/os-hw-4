...
ObserverList *observer_head = NULL;

void appendToObserverList(packet_data *data) {
    ObserverList *newNode = malloc(sizeof(ObserverList));
    newNode->data = data;
    newNode->next = NULL;

    if (observer_head == NULL) {
        observer_head = newNode;
    } else {
        ObserverList *temp = observer_head;
...
void *seller(void *arg) {
    ...
    while (1) {
        ...
        char buf[256]; 
        snprintf(buf, sizeof(buf), "Client %s:%d has been served\n", clientIP, clientPort);
        broadcast_to_observers(buf);         
...
void* handle_client(void* arg) {
    ...
    inet_ntop(AF_INET, &(data->client_addr.sin_addr), clientIP, INET_ADDRSTRLEN);
        ...
        if (strcmp(data->buffer, "1\n") == 0) {
            ...
            char buf[256]; 
            snprintf(buf, sizeof(buf), "Client %s:%d got in line 1\n", clientIP, clientPort);
            broadcast_to_observers(buf);
            ...
            char buf[256]; 
            snprintf(buf, sizeof(buf), "Client %s:%d got in line 2\n", clientIP, clientPort);
            broadcast_to_observers(buf);           
            ...
            char buf[256]; 
            snprintf(buf, sizeof(buf), "Client %s:%d got in line 3\n", clientIP, clientPort);
            broadcast_to_observers(buf);            
            ...
            char buf[256]; 
            snprintf(buf, sizeof(buf), "Client %s:%d left\n", clientIP, clientPort);
            broadcast_to_observers(buf); 
        ...
        } else if (strcmp(data->buffer, "observe\n") == 0) {
            printf("Client %s:%d is observing\n", clientIP, clientPort);
            char buf[256]; 
            snprintf(buf, sizeof(buf), "Client %s:%d is observing\n", clientIP, clientPort);
            broadcast_to_observers(buf);
            if (sendto(data->sock, "observing\n", strlen("observing\n"), 0, (struct sockaddr*)&data->client_addr, sizeof(data->client_addr)) < 0) {
                perror("sendto failed");
                close(data->sock);
                return NULL;
            }
            appendToObserverList(data);
...
void broadcast_to_observers(const char* message) {
    ObserverList* temp = observer_head;
    while(temp != NULL) {
        if(sendto(temp->data->sock, message, strlen(message), 0, (struct sockaddr*)&temp->data->client_addr, sizeof(temp->data->client_addr)) < 0) {
            perror("sendto failed");
